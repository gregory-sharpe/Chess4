...
class MCTS(object):
    ...
    def _playout(self, state):
        ...
        if not state.gameOver:
            ...
            winner = ChessEngine.TEAMS[state.gameOutcome.index(1)]
            if winner == -1:  # tie
                leaf_value = 0.0
            else:
                leaf_value = (
                    1.0 if winner == state.turn else -1.0
                )
        #end3 = time.time()
        #print(end3-start3)
...
class MCTSPlayer(object):
    ...
    def get_action(self, board:ChessEngine.GameState, temp=1e-3, return_prob=0):
